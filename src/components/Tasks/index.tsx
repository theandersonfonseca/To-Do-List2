...
const Tasks = () => {
  ...
  const handleTaskEdit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch({type: Actions.editTask, payload: {oldValue: taskValueToEdit, newValue: inputEditingValue}})
  ...
  return (
    <S.Wrapper>
      {state.lists.find((list) => list.name === state.currentList)?.tasks.map(task => 
        <S.Task key={task.id}>
          {!task.editing ? 
            <>
              <S.Check className={task.complete ? 'checked' : ''} onClick={handleCheckTask}>
                {task.complete ? <S.CheckIcon className="far fa-check"></S.CheckIcon> : ''}
              </S.Check>

              <S.Description className={task.complete ? 'checked' : ''}>{task.description}</S.Description>

              <S.Controls>
                {!task.complete && <S.Edit className="fas fa-edit" onClick={() => showTaskEdit(task.description)}></S.Edit>}
                <S.Remove className="fas fa-trash" onClick={removeTask}></S.Remove> 
              </S.Controls>
            </> :
