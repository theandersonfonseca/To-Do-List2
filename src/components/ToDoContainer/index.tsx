...
const ToDoContainer = ({children}: ToDoContainerType) => {
  ...
  return (
    <S.Wrapper>
      {children}
      <S.WrapperControls>
        <S.RemoveCompleteTasks onClick={removeCompleteTasks}>Remover tarefas completas</S.RemoveCompleteTasks>
        {state.currentList !== 'Lista Principal' && <S.RemoveList onClick={removeList}>Remover Lista</S.RemoveList>}
      </S.WrapperControls>
    </S.Wrapper>
  );
};
